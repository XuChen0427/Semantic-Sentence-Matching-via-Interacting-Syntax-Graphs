...
from .lap_solvers.sinkhorn import Sinkhorn
...
class GNNLayer(nn.Module):
    ...
    def forward(self, A, W, x, n1=None, n2=None, norm=True):
        ...
        length = int(math.sqrt(x.shape[1]))

        # if self.classifier is not None:
        #     #assert max(n1) * max(n2) == x.shape[1]
        #     x3 = self.classifier(x2)
        #     #n1_rep = torch.repeat_interleave(torch.LongTensor(n1), self.sk_channel, dim=0)
        #     #n2_rep = torch.repeat_interleave(torch.LongTensor(n2), self.sk_channel, dim=0)
        #     #x4 = x3.permute(0,2,1).reshape(x.shape[0] * self.sk_channel, length, length).transpose(1, 2)
        #     #x5 = self.sk(x4, n1_rep, n2_rep, dummy_row=True).transpose(2, 1).contiguous()
        #
        #     x6 = x3.reshape(x.shape[0], self.sk_channel, x.shape[1]).permute(0, 2, 1)
