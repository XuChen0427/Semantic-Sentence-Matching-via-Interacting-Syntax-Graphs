...
class ProjectAffinity(nn.Module):
    def __init__(self, d):
        super(ProjectAffinity, self).__init__()
    ...
    def forward(self, X, Y):
        #assert X.shape[2] == Y.shape[2] == 2 * self.d
        lambda1 = self.relu(self.lambda1 + self.lambda1.transpose(0, 1))
        lambda2 = self.relu(self.lambda2 + self.lambda2.transpose(0, 1)) 
        weight = torch.cat((torch.cat((lambda1, lambda2)),
                            torch.cat((lambda2, lambda1))), 1)
        Me = torch.matmul(X, weight)
        Me = torch.matmul(Me, Y.transpose(1, 2))
        #Mp = torch.matmul(Ux, Uy.transpose(1, 2))
...
class PointAffinity(nn.Module):
    ...
    """
    def __init__(self, d):
        super(PointAffinity, self).__init__()
        self.d = d
        self.lambda1 = Parameter(Tensor(self.d, self.d))
        self.lambda2 = Parameter(Tensor(self.d, self.d))
        self.relu = nn.ReLU()  # problem: if weight<0, then always grad=0. So this parameter is never updated!
    ...
    def forward(self, X, Y, w1=1, w2=1):
        ...
        weight = torch.cat((torch.cat((lambda1, lambda2)),
                            torch.cat((lambda2, lambda1))), 1)
        Me = torch.matmul(X, weight)
        Me = torch.matmul(Me, Y.transpose(0, 1))
